...
# 从 builder 镜像中的把 /convo/convo_app 拷贝到当前目录
COPY --from=builder /convo/convo_app /
...
# ENTRYPOINT ["/convo_app"]

# 镜像构建命令(终端命令)
# docker build . -t docker_convo

# 启动容器的命令(联合启动容器命令-终端命令): 此时需要将配置文件中 mysql 和 redis 中的 host 分别改为 mysql 和 redis
# docker run --name docker_convo --link=mysql:mysql --link=redis:redis -p 9090:9090 docker_convo